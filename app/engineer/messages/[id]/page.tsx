...
export default function EngineerMessageDetailPage() {
                  ...
                  >
                    ...
                    <p
                      className={`text-xs mt-2 ${
                        isOwn ? 'text-cyan-200' : 'text-midnight-500'
                      }`}
                    >
        ...
        <form onSubmit={handleSend} className="p-4 border-t border-midnight-700/50 bg-midnight-800/30">
          <div className="flex gap-3">
            <input
              ...
              disabled={sending}
              className="flex-1 px-4 py-3 rounded-xl bg-midnight-800/50 border border-midnight-600/50 text-white placeholder:text-midnight-500 focus:outline-none focus:border-cyan-glow/50 focus:ring-2 focus:ring-cyan-glow/20 transition-all disabled:opacity-50"
            />
            <button
              type="submit"
              disabled={sending || !newMessage.trim()}
              className="px-6 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-cyan-600 text-white font-medium hover:from-cyan-400 hover:to-cyan-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-2 shadow-lg shadow-cyan-500/20"
            >
              {sending ? (
